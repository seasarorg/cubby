...
<input type="checkbox" name="${name}" id="${name}-${status.index}" value="${item.value}" ${f:checked(item.value,checkedValue)}/><label for="${name}-${status.index}" style="cursor:pointer">${item.name}</label></c:forEach>