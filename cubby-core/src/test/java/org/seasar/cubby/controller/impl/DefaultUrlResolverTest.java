...
public class DefaultUrlResolverTest extends TestCase {
	...
	public void testResolve() throws Exception {
		...
		ActionMethod m = resolver.resolve("/sample/1");
		assertEquals(ClassUtils.getMethod(Sample1Controller.class, "show",null), m.getMethod());
		
		// format:/cubby-dispacher/(controllerClassName)/(actionMethod)
		m = resolver.resolve("/cubby-dispacher/org.seasar.cubby.seasar.Sample1Controller/show");
		assertEquals(ClassUtils.getMethod(Sample1Controller.class, "show",null), m.getMethod());
	}
