...
<input type="radio" name="${name}" id="${name}-${status.index}" value="${item.value}" ${f:checked(item.value, value)}/><label for="${name}-${status.index}" style="cursor:pointer">${item.name}</label>
</c:forEach>
